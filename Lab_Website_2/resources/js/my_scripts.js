...
*/
	function loadStatsPage(){
		var TW=0;
		var x=document.getElementById("stats_table").rows[2].cells;
		for(let i=0;i<12;i++){
			x=document.getElementById("stats_table").rows[2+i].cells;
			var Cu=parseInt(x[2].innerHTML);
			var Op=parseInt(x[3].innerHTML);

			if(Cu>=Op){
				x[4].innerHTML="Cu Boulder";
				TW++;
			}
			else{
		...
		document.getElementById("wins").innerHTML=TW;
		document.getElementById("losses").innerHTML=12-TW;
	}
/*
...
*/
	function loadPlayersPage(){
		...
		for(i=0;i<players.length;i++){
			string+='<a class="dropdown-Item" href="#" onclick="switchPlayers(' + i + ')">' + players[i].name + '</a>' + '<br>';
		}
